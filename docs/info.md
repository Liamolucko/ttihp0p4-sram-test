...
(This is copied from the ttihp0p2 SRAM test, since this should behave identically.)

### Implementation details

This macro does not integrate very nicely with Tiny Tapeout's sg13cmos5l setup. It assumes that it can freely take up layers Metal1-Metal4, and you will power it using one of the remaining metal layers. However, on sg13cmos5l, there is only one remaining metal layer: TopMetal1, which is reserved for use by Tiny Tapeout's multiplexer.

As a result, the design had to be modified slightly to extend the macro's power rails such that they could be connected to a power ring, and from there to the rest of the tile's power distribution network.
